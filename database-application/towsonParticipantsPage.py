...
class TowsonParticipantsPage(tkinter.Frame):
    ...
    def populate_window(self, controller):
        """ 
            ...
            - controller : the application
        """
        # Query: Get the name, gender, & age of all Towson University participants (faculty/students)
        query = """ SELECT Name, Sex, Age FROM Students UNION ALL
                    SELECT Name, Sex, Age FROM Faculty_Staff
                    ORDER BY Name """

        self.create_header(controller, 'All participants from Towson University (faculty & students)')
